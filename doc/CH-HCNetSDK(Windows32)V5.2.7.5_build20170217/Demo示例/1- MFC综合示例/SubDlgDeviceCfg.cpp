...
void CSubDlgDeviceCfg::CurCfgUpdate()
{
    ...
    if (g_struDeviceInfo[m_iDeviceIndex].byCharaterEncodeType == ENUM_MEM_CHAR_ENCODE_UTF8)
    {
        ...
        memcpy(szInputName, m_struDeviceCfgV50.sDVRName, NET_DEV_NAME_LEN);
    ...
    {
        memcpy(cTemp, m_struDeviceCfgV50.sDVRName, NET_DEV_NAME_LEN);
    }
	...
	ZeroMemory(cTemp, 200);
    memcpy(cTemp, m_struDeviceCfgV50.sSerialNumber, SERIALNO_LEN);
	m_csDeviceSerialNo.Format("%s", cTemp);
    memcpy(cTemp, m_struDeviceCfgV50.byDevTypeName, sizeof(m_struDeviceCfgV50.byDevTypeName));
...
void CSubDlgDeviceCfg::OnBnClickedBtnRemoteNetOk()
{
    ...
    {
        char szInputName[200] = {0};
        memcpy(szInputName, m_csDeviceName, NAME_LEN);	
    ...
    {
        memcpy(m_struDeviceCfgV50.sDVRName, m_csDeviceName, NAME_LEN);
    }
