...
extern void InfoMinorTypeMap(const NET_DVR_LOG_V30 &stLogInfo, CString &szTemp);
extern void EventMinorTypeMap(const NET_DVR_LOG_V30 &stLogInfo, CString &szTemp);
extern void OperationMinorTypeMap(const NET_DVR_LOG_V30 &stLogInfo, CString &szTemp);
extern void ExceptionMinorTypeMap(const NET_DVR_LOG_V30 &stLogInfo, CString &szTemp);
extern void AlarmMinorTypeMap(const NET_DVR_LOG_V30 &stLogInfo, CString &szTemp);
extern void ParaTypeMap(const NET_DVR_LOG_V30 &stLogInfo, CString &szTemp);
...
void CClientDemoDlg::OnMenuChannelInfo()
{
	...
	if (iChanIndex >= g_struDeviceInfo[iDeviceIndex].iDeviceChanNum)
	{
...
void *CClientDemoDlg::DoLoginEx(void *lpParam)
{
	...
	if (g_struDeviceInfo[iDeviceIndex].lLoginID < 0)
	{
		...
		if (g_struDeviceInfo[iDeviceIndex].iDeviceType == DS_B20_MSU_NP || \
            ...
            g_struDeviceInfo[iDeviceIndex].iDeviceType == DS_B21_MCU_NP_CLASS) //B20/B21主控
		{
			int nAllChanNums = 0;
			int nPreChanNums = 0;
...
void CClientDemoDlg::ProcPdcAlarm(WPARAM wParam, LPARAM lParam)
{
    ...
    if (struPDCAlarmInfo.byMode == 0 )
    {
        ...
        sprintf(szInfoBuf, "Abs[%4.4d%2.2d%2.2d%2.2d%2.2d%2.2d] Rel[%4.4d%2.2d%2.2d%2.2d%2.2d%2.2d] DevIP:[%s]Port[%d]Channel[%d] IvmsChannel[%d]IvmsChannelEx[%d] single frame:Channel[%d]LeaveNum[%d]EnterNum[%d]Smart[%d] BrokenNetHttp[%d]",
            ...
            struPDCAlarmInfo.byChannel, struPDCAlarmInfo.dwLeaveNum, struPDCAlarmInfo.dwEnterNum, struPDCAlarmInfo.bySmart, struPDCAlarmInfo.byBrokenNetHttp);//2014-03-21
    }
    else if (struPDCAlarmInfo.byMode == 1)
    {
		sprintf(szInfoBuf,"DevIP:[%s]Port[%d]Channel[%d]:time range:Start Y[%d]M[%d]D[%d]H[%d]M[%d]S[%d]  End Y[%d]M[%d]D[%d]H[%d]M[%d]S[%d]IvmsChannel[%d]IvmsChannelEx[%d]Channel[%d]LeaveNum[%d]EnterNum[%d]Smart[%d]BrokenNetHttp[%d]", 
            struPDCAlarmInfo.struDevInfo.struDevIP.sIpV4,struPDCAlarmInfo.struDevInfo.wPort,struPDCAlarmInfo.struDevInfo.byChannel,
...
void CClientDemoDlg::ProcRuleAlarm(WPARAM wParam, LPARAM lParam)
{
    ...
    char csRuleName[32] = {0};
    memcpy(csRuleName, struVcaRuleAlarm.struRuleInfo.byRuleName, 32);
	//行为分析结果上报结构
	sprintf(szInfoBuf,"Smart:[%d] DevIP:[%s]TargetID[%d]RuleID[%d]RuleName[%s]AbsTime[%d]piclen[%d]PicType[%d]RelAlarmPicNum[%d]AlarmID[%d]IvmsChannel[%d]IvmsChannelEx[%d]EventTypeEx[%d]",
	...
	case VCA_ANSWER:
		{
			...
			for (i = 0; i < (int)tempPolygon.dwPointNum ;i ++)
			{
                ...
                sprintf(szInfoBuf, "%s ZoomOver[%d]", szInfoBuf, struVcaRuleAlarm.struRuleInfo.uEventParam.struAnswer.byZoomOver);
				OutputDebugString(szInfoBuf);
				g_pMainDlg->AddLog(iDeviceIndex,ALARM_INFO_T,szInfoBuf);
			}
...
void CClientDemoDlg::ProcAidAlarmV41(WPARAM wParam,LPARAM lParam)
{
	...
	CString str = "";
    sprintf(szInfoBuf, "Aid AlarmV41 IvmsChan[%d]ID[%d]Name[%s]Type[%d]newType[%s] LaneNo[%d] Abs[%4.4d%2.2d%2.2d%2.2d%2.2d%2.2d]Dev IP[%s]Port[%d]Chan[%d] \
...
void CClientDemoDlg::ProcTfsAlarm(WPARAM wParam,LPARAM lParam)
{
    ...
    sprintf(szInfoBuf, "Tfs Alarm IvmsChan[%d]ID[%d]Name[%s]Type[%d]Abs[%4.4d%2.2d%2.2d%2.2d%2.2d%2.2d]DevIP[%s]Port[%d]Chan[%d]SpecificVehicleType[%d]\
        LaneNo[%d]Direction Start[%0.3f,%0.3f]Stop[%0.3f,%0.3f]SceneID[%d]SceneName[%s]Direction[%d]PTZ[%d,%d,%d]IllegalType[%u]IllegalTime[%d],MonitorSiteID[%s]\
        ...
        PilotSunVisor[%d]CopilotSunVisor[%d]PilotCall[%d]IllegalCode[%s]",
        ...
        struTfsAlarm.byCopilotSunVisor, struTfsAlarm.byPilotCall, struTfsAlarm.byIllegalCode);
	...
	for (i = 0; i < 8; i++)
	{
		if (struTfsAlarm.struPicInfo[i].dwDataLen > 0 && struTfsAlarm.struPicInfo[i].pBuffer != NULL)
		{
			...
			if (struTfsAlarm.struPicInfo[i].byDataType == 0)
			{
				...
				sprintf(cFilename, "%s\\%sLocalTime[%s]_%d.jpg",cFilename, cPicType, chTime, i);
			} 
			else if(struTfsAlarm.struPicInfo[i].byDataType == 1)
			{
				...
				sprintf(cFilename, "%s\\%sURL_LocalTime[%s]_%d.txt", cFilename, cPicType, chTime, i);
			}
...
void CClientDemoDlg::ProcFaceSnapAlarm(WPARAM wParam, LPARAM lParam)
{
    ...
    sprintf(szInfoBuf, "Face Snap alarm Abs[%4.4d%2.2d%2.2d%2.2d%2.2d%2.2d] FaceID[%d] Smart[%d] AgeGroup[%d] Sex[%d] EyeGlass[%d] \
        ...
        struFaceSnapAlarm.struDevInfo.byIvmsChannel, struFaceSnapAlarm.wDevInfoIvmsChannelEx);
    AddLog(m_iCurDeviceIndex, ALARM_INFO_T, szInfoBuf);
...
// 			struFaceDetectionAlarm.struFacePic[i].fY != 0)
	...
	sprintf(szInfoBuf, "[Detection Face Snap alarm] Abs[%4.4d%2.2d%2.2d%2.2d%2.2d%2.2d] RelativeTime[%4.4d%2.2d%2.2d%2.2d%2.2d%2.2d] Dev[ip:%s,port:%d,ivmsChan:%d] IvmsChannelEx[%d] \n",\
        struAbsTime.dwYear, struAbsTime.dwMonth, struAbsTime.dwDay, struAbsTime.dwHour,struAbsTime.dwMinute, struAbsTime.dwSecond, \
		struRelativeTime.dwYear, struRelativeTime.dwMonth, struRelativeTime.dwDay, struRelativeTime.dwHour,struRelativeTime.dwMinute, struRelativeTime.dwSecond, \
        ...
        struFaceDetectionAlarm.wDevInfoIvmsChannelEx/*szPicFace*/);
...
void CClientDemoDlg::ProcFaceSnapMatchAlarm(WPARAM wParam, LPARAM lParam)
{
    ...
    sprintf(szInfoBuf, "Face Snap Match alarm Abs[%4.4d%2.2d%2.2d%2.2d%2.2d%2.2d] FaceID[%d] BlackList ID[%d] StorageIP[%s]-StoragePort[%d] Sex[%d] Glasses[%d] Age[%d] MatchPicNum[%d] struRegion[%0.1f, %0.1f, %0.1f, %0.1f] PicTransType[%d]\n",
        struAbsTime.dwYear, struAbsTime.dwMonth, struAbsTime.dwDay, struAbsTime.dwHour,
        ...
        struFaceSnapMatchAlarm.struSnapInfo.byAge, struFaceSnapMatchAlarm.byMatchPicNum, \
        struFaceSnapMatchAlarm.struRegion.fX, struFaceSnapMatchAlarm.struRegion.fY, struFaceSnapMatchAlarm.struRegion.fWidth, struFaceSnapMatchAlarm.struRegion.fHeight, struFaceSnapMatchAlarm.byPicTransType);
    AddLog(m_iCurDeviceIndex, ALARM_INFO_T, szInfoBuf);
...
//         sprintf(chTime,"%4.4d%2.2d%2.2d%2.2d%2.2d%2.2d%3.3d",t.wYear,t.wMonth,t.wDay,t.wHour,t.wMinute,t.wSecond,t.wMilliseconds);
    ...
    if (struFaceSnapMatchAlarm.dwSnapPicLen > 0 && struFaceSnapMatchAlarm.pSnapPicBuffer != NULL)
    {
        ...
        if (struFaceSnapMatchAlarm.byPicTransType)
            ...
            sprintf(cFilename, "%s\\MatchSnapBigPic.jpg", cFilename);
        hFile = CreateFile(cFilename, GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
        if (hFile == INVALID_HANDLE_VALUE)
        {
            return;
...
void CClientDemoDlg::ProcAlarmHostCIDAlarm(WPARAM wParam, LPARAM lParam)
{
	...
	{
		sprintf(szLanCn, "%s, 防区号：%d", szLanCn, struAlarm.wDefenceNo);
...
void CClientDemoDlg::ProcDefocus(WPARAM wParam, LPARAM lParam)
{
    ...
    sprintf(szInfoBuf, "%s: IvmsChannel:%d]IvmsChannelEx[%d]\n", szLan, \
        struDefocus.struDevInfo.byIvmsChannel, struDefocus.wDevInfoIvmsChannelEx);
    AddLog(iDeviceIndex, ALARM_INFO_T, szInfoBuf);
}
...
void CClientDemoDlg::ProcITSPlateAlarm(WPARAM wParam, LPARAM lParam)
{
	...
	sprintf(szInfoBuf, "ITS Plate Alarm Analysis[%d]YellowLabel[%d]DangerousVeh[%d]Region[%d]Country[%d]MatchNo[%d]DriveChan[%d]IllegalType[%d]IllegalSubType[%s]\
	  MonitoringSiteID[%s]DeviceID[%s]Dir[%d]PicNum[%d]License[%s]RadarState[%d]Speed[%d]VehicleLogoRecog[%d]\
	  VehicleSubLogoRecog[%d] DetSceneID[%d] VehicleType[%d] DetectType[%d]\
	  Type[%d] DataType[%d] CloseUpType[%d] EntireBelieve[%d] Channel NO[%d] LicenseLen[%d] PlateRect[x-%f y-%f w-%f h-%f]", \
	    ...
	    struPlateResult.byDir, struPlateResult.dwPicNum,struPlateResult.struPlateInfo.sLicense, struPlateResult.struVehicleInfo.byRadarState, struPlateResult.struVehicleInfo.wSpeed,\
		...
		struPlateResult.struPicInfo->byType, struPlateResult.struPicInfo->byDataType, \
        struPlateResult.struPicInfo->byCloseUpType, struPlateResult.struPlateInfo.byEntireBelieve, struPlateResult.byChanIndexEx * 256 + struPlateResult.byChanIndex, struPlateResult.struPlateInfo.byLicenseLen, struPlateResult.struPlateInfo.struPlateRect.fX, struPlateResult.struPlateInfo.struPlateRect.fY, struPlateResult.struPlateInfo.struPlateRect.fWidth, struPlateResult.struPlateInfo.struPlateRect.fHeight);
    g_pMainDlg->AddLog(iDeviceIndex, ALARM_INFO_T, szInfoBuf);	
...
void CClientDemoDlg::ProcITSPlateAlarmV50(WPARAM wParam, LPARAM lParam)
{
    ...
    sprintf(szInfoBuf, "ITS Plate Alarm V50;IsOverWeight[%d]AxleNum[%d]OverWeight[%f]Weight[%f]LimitWeight[%f]AxleLen[%f]DevDescInfo[%s]\
          ...
          struPlateResult.struWeightResult.byIsOverWeight, struPlateResult.struWeightResult.byAxleNum,
...
void CClientDemoDlg::ProcVehicleControl(WPARAM wParam, LPARAM lParam)
{
    ...
    sprintf(szInfoBuf, "%s:  byListType[%d] byPlateType[%d] byPlateColor[%d] sLicense[%s] dwChannel[%d]", \
        szLan, pVehicle->byListType, pVehicle->byPlateType, pVehicle->byPlateColor, pVehicle->sLicense, pVehicle->dwChannel);
    AddLog(iDeviceIndex, ALARM_INFO_T, szInfoBuf);
...
void CClientDemoDlg::ProFirDetectionAlarm(WPARAM wParam, LPARAM lParam)
{
    ...
    sprintf(szInfoBuf, "%s, RelativeTime:%d, AbsTime:%d, PTZ{PanPos:%d, TiltPos:%d, ZoomPos:%d}, PicDataLen:%d, DevInfo{DevIP:%s, Port:%d, Channel:%d, IvmsChannel:%d,  IvmsChannelEx:%d}, FireMaxTemperature:%d, TargetDistance:%d, fireRectInfo{fX:%f,fY:%f,fWidth%f,fHeight%f}, fireMaxTemperaturePoint{fX:%f,fY:%f}, PTZEx{P:%f,T:%f,Z:%f,F:%d},AlarmSubType:%d",
        szLan, struFireDetection.dwRelativeTime, struFireDetection.dwAbsTime, struFireDetection.wPanPos, struFireDetection.wTiltPos, struFireDetection.wZoomPos, \
...
{  
	...
	{
		sprintf(szInfoBuf, "Video Intercom Alarm Time[%4.4d%2.2d%2.2d%2.2d%2.2d%2.2d] DevNumber[%s] AlarmType[%d] ZoneName[%s] ZoneIndex[%d] ZoneType[%d]", 
	...
	{
		sprintf(szInfoBuf, "Video Intercom Alarm Time[%4.4d%2.2d%2.2d%2.2d%2.2d%2.2d] DevNumber[%s] AlarmType[%d]", 
			struVideoIntercomAlarm.struTime.wYear, struVideoIntercomAlarm.struTime.byMonth, struVideoIntercomAlarm.struTime.byDay,
			struVideoIntercomAlarm.struTime.byHour, struVideoIntercomAlarm.struTime.byMinute, struVideoIntercomAlarm.struTime.bySecond, 
...
void CClientDemoDlg::ProcFacecaptureStatistics(WPARAM wParam, LPARAM lParam)//人脸抓拍统计上传
{
	...
	g_pMainDlg->AddLog(iDeviceIndex, ALARM_INFO_T, szInfoBuf);
    char szTmpCN[200] = {0};
    sprintf(szTmpCN, "统计人数：%d,少年：%d, 青年：%d，中年：%d， 老年：%d， 男：%d， 女：%d", struFacecapture.dwPeopleNum, struFacecapture.struAgeGroupParam.dwTeenage, \
    ...
    char szTmpEN[200] = {0};
    sprintf(szTmpEN, "PeopleNum:%d，Teen：%d, Youth：%d，Midlife：%d， Elderly：%d， male：%d，female：%d", struFacecapture.dwPeopleNum, struFacecapture.struAgeGroupParam.dwTeenage, \
        struFacecapture.struAgeGroupParam.dwYouth, struFacecapture.struAgeGroupParam.dwMidLife, struFacecapture.struAgeGroupParam.dwElderly,\
        struFacecapture.struSexGroupParam.dwMale, struFacecapture.struSexGroupParam.dwFemale);
...
void CClientDemoDlg::ProcShipsDetAlarm(WPARAM wParam, LPARAM lParam)
{
    ...
    sprintf(szInfoBuf, "%s: IPv4[%s] IPv6[%s] Port[%d] Channel[%d] IvmsChannel[%d]  IvmsChannelEx[%d] RelativeTime[%d] AbsTime[%d] ShipsNum[%d] ShipsNumHead[%d] ShipsNumEnd[%d]", \
    ...
    {
        sprintf(szInfoBuf, "%s: shipsNum[%d] ShipsLength[%0.1f] ShipsHeight[%0.1f] ShipsWidth[%0.1f] ShipsSpeed[%0.1f] ShipsDirection[%d] ShipsDetState[%d]", \
            szLan, i + 1, struShipsDetection.struShipInfo[i].fShipsLength, struShipsDetection.struShipInfo[i].fShipsHeight, struShipsDetection.struShipInfo[i].fShipsWidth, \
...
void CClientDemoDlg::ProThermometryAlarm(WPARAM wParam, LPARAM lParam)
{
    ...
    {
        sprintf(szInfoBuf, "%s: Channel:%d, RuleID:%d, ThermometryUnit:%d, PresetNo:%d, RuleTemperature:%.1f, CurrTemperature:%.1f, PTZ Info[Pan:%f, Tilt:%f, Zoom:%f], AlarmLevel:%d, \
                                                                  AlarmType:%d, AlarmRule:%d, RuleCalibType:%d,  Point[x:%f, y:%f], PicLen:%d, ThermalPicLen:%d, ThermalInfoLen:%d", szLan, \
                                                                  struThermometryAlarm.dwChannel, struThermometryAlarm.byRuleID, struThermometryAlarm.byThermometryUnit, struThermometryAlarm.wPresetNo, \
    ...
    else if (1 == struThermometryAlarm.byRuleCalibType || 2 == struThermometryAlarm.byRuleCalibType)
    {
        ...
        sprintf(szInfoBuf, "%s: Channel:%d, RuleID:%d,HighestPoint[x:%f, y:%f],ThermometryUnit:%d, PresetNo:%d, RuleTemperature:%.1f, CurrTemperature:%.1f, PTZ Info[Pan:%f, Tilt:%f, Zoom:%f], AlarmLevel:%d, \
                                                                  AlarmType:%d, AlarmRule:%d, RuleCalibType:%d,  Region[%s], PicLen:%d, ThermalPicLen:%d, ThermalInfoLen:%d", szLan, \
																  struThermometryAlarm.dwChannel, struThermometryAlarm.byRuleID, struThermometryAlarm.struHighestPoint.fX, struThermometryAlarm.struHighestPoint.fY, struThermometryAlarm.byThermometryUnit, struThermometryAlarm.wPresetNo, \
...
void CClientDemoDlg::ProThermometryDiffAlarm(WPARAM wParam, LPARAM lParam)
{
    ...
    if (0 == struThermometryAlarm.byRuleCalibType)
    {
        sprintf(szInfoBuf, "%s: Channel:%d, AlarmID1:%d, AlarmID2:%d, PresetNo:%d, RuleTemperatureDiff:%.1f, CurTemperatureDiff:%.1f, \
                ...
                struThermometryAlarm.dwChannel, struThermometryAlarm.byAlarmID1, struThermometryAlarm.byAlarmID2, struThermometryAlarm.wPresetNo, \
    ...
    else if (1 == struThermometryAlarm.byRuleCalibType || 2 == struThermometryAlarm.byRuleCalibType)
    {
        ...
        sprintf(szInfoBuf, "%s: Channel:%d, AlarmID1:%d, AlarmID2:%d, PresetNo:%d, RuleTemperatureDiff:%.1f, CurTemperatureDiff:%.1f, AlarmLevel:%d, AlarmType:%d, AlarmRule:%d, RuleCalibType:%d,  \
...
void CClientDemoDlg::ProcessEventWithJsonData(WPARAM wParam, LPARAM lParam)
{
	...
	while (1) {
		...
		char cFilename[256] = { 0 };
		sprintf(cFilename, "%s\\%s.%s", g_struLocalParam.chPictureSavePath, chTime, type);
