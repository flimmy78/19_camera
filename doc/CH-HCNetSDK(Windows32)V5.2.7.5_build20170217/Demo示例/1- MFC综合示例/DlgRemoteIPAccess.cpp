	...
	if (g_pMainDlg->DoGetDeviceResoureCfg(m_dwDevIndex, iGroupNO))
	{
		...
		for (i = 0; i < MAX_CHANNUM_V30; i++)
		{
			...
			if (m_struIPParaCfgV40.byAnalogChanEnable[i])
			{
				...
				m_listIPChan.SetItemData(iIndex, ANALOG_CHAN_ENABLE*1000+i  + m_struIPParaCfgV40.dwStartDChan);
			}
			else
			{
				g_StringLanType(szLan, "不可用", "Disabled");
				m_listIPChan.SetItemText(i, 2, szLan);
				m_listIPChan.SetItemData(iIndex, ANALOG_CHAN_DISABLE*1000+i + m_struIPParaCfgV40.dwStartDChan);
			}
			iIndex++;
		}
...
void CDlgRemoteIPAccess::OnBtnDelete() 
{
	...
	DeleteDevIPByIP(m_struIPParaCfgV40.struStreamMode[iIPChanIndex].uGetStream.struChanInfo.byIPID + \
        m_struIPParaCfgV40.struStreamMode[iIPChanIndex].uGetStream.struChanInfo.byIPIDHigh*0xff,iIPChanIndex);
	...
	m_listIPChan.SetItemData(m_iCurSel, IP_CHAN_ENABLE*1000+iIPChanIndex + m_struIPParaCfgV40.dwStartDChan/*m_iCurChanNO*/);
...
void CDlgRemoteIPAccess::OnBtnAddIPChan() 
{
	...
	m_struIPParaCfgV40.struStreamMode[iIPChanIndex].byGetStreamType =  m_comboStreamType.GetCurSel(); //direct connect device.
	//clear device info
		...
		if (!bFind)   //no find
		{
			...
			/*SetDeviceAddress(&(m_struIPParaCfgV40.struIPDevInfo[iDevID]), m_csDevIP.GetBuffer(0));*/
			 ...
			 if (m_struProtoList.dwProtoNum < IPC_PROTOCOL_NUM)
			 {
				 ...
				 {
					 if (dwProtoType == m_struProtoList.struProto[i].dwType)
					 {
						 m_comboProType.SetCurSel(i); //protocol	
						 if (!strcmp((char*)m_struProtoList.struProto[i].byDescribe,"GB28181"))
						 {
			...
			memcpy(m_struIPParaCfgV40.struIPDevInfo[iDevID].sUserName, m_ceDevUserName.GetBuffer(0), NAME_LEN);
			memcpy(m_struIPParaCfgV40.struIPDevInfo[iDevID].sPassword, m_csDevPwd.GetBuffer(0), PASSWD_LEN);
			m_struIPParaCfgV40.struIPDevInfo[iDevID].wDVRPort = (WORD)m_uDevPort;
			m_struIPParaCfgV40.struStreamMode[iIPChanIndex].uGetStream.struChanInfo.byIPID = (iDevID + iGroupNO*MAX_IP_DEVICE_V40  +1) % 256;
			m_struIPParaCfgV40.struStreamMode[iIPChanIndex].uGetStream.struChanInfo.byIPIDHigh = (iDevID  + iGroupNO*MAX_IP_DEVICE_V40 +1)/256;
		}
		else
		{ 
			m_struIPParaCfgV40.struStreamMode[iIPChanIndex].uGetStream.struChanInfo.byIPID = (iIPChanIndex + 1 + iGroupNO*MAX_IP_DEVICE_V40) % 256;
			m_struIPParaCfgV40.struStreamMode[iIPChanIndex].uGetStream.struChanInfo.byIPIDHigh = (iIPChanIndex + 1 + iGroupNO*MAX_IP_DEVICE_V40) /256;
	...
	{
		m_struIPParaCfgV40.struStreamMode[iIPChanIndex].uGetStream.struIPChan.byEnable = FALSE; //Is online?
		m_struIPParaCfgV40.struStreamMode[iIPChanIndex].uGetStream.struIPChan.dwChannel = m_iIPDevChan;
		int iDevID = iIPChanIndex + iGroupNO * MAX_IP_DEVICE_V40 + 1;
		m_struIPParaCfgV40.struStreamMode[iIPChanIndex].uGetStream.struIPChan.wIPID = iDevID;
		m_struIPParaCfgV40.struStreamMode[iIPChanIndex].uGetStream.struIPChan.byTransMode = m_comboIPStreamType.GetCurSel();
		m_struIPParaCfgV40.struStreamMode[iIPChanIndex].uGetStream.struIPChan.byTransProtocol = m_comboTransProtocol.GetCurSel();
	}
...
// 	m_struIPParaCfgV40.struIPDevInfo[iIPChanIndex].wDVRPort = (WORD)m_uDevPort;
	...
	m_listIPChan.SetItemData(m_iCurSel, IP_CHAN_ENABLE*1000 + iIPChanIndex + m_struIPParaCfgV40.dwStartDChan);
...
void CDlgRemoteIPAccess::OnBtnEnable() 
{
	...
	if (m_bCurEnable)
	{
		g_StringLanType(szLan, "禁用", "Disable");
		m_listIPChan.SetItemText(m_iCurSel, 2, szLan);
		m_listIPChan.SetItemData(m_iCurSel, ANALOG_CHAN_DISABLE*1000+dwCurChanIndex + m_struIPParaCfgV40.dwStartDChan);
	...
	else
	{
		g_StringLanType(szLan, "可用", "Enable");
		m_listIPChan.SetItemText(m_iCurSel, 2, szLan);
		m_listIPChan.SetItemData(m_iCurSel, ANALOG_CHAN_ENABLE*1000+dwCurChanIndex + m_struIPParaCfgV40.dwStartDChan);
...
void CDlgRemoteIPAccess::SetChannelInfobyStreamType(BOOL bAnalog)
{
		...
		if (iIPChanIndex >= 0)
		{
			...
			if (dwID > 0)
            {
                ...
                if (m_struProtoList.dwProtoNum < IPC_PROTOCOL_NUM)
                {
                    for (int i = 0; i<m_struProtoList.dwProtoNum; i++)
                    {
                        if (lpIPParaCfgV40Temp->struIPDevInfo[dwIPDevIndex].byProType == m_struProtoList.struProto[i].dwType)
                        {
                            m_comboProType.SetCurSel(i); //protocol	
							if (!strcmp((char*)m_struProtoList.struProto[i].byDescribe,"GB28181"))
							{
	...
	case 6: //Get stream from device.  
        if (iIPChanIndex >= 0)
		{
			int dwID = m_struIPParaCfgV40.struStreamMode[iIPChanIndex].uGetStream.struIPChan.wIPID;
			if (dwID > 0 && dwID <= MAX_IP_DEVICE_V40)
			{
				...
				m_iIPDevChan = m_struIPParaCfgV40.struStreamMode[iIPChanIndex].uGetStream.struIPChan.dwChannel;
				m_comboTransProtocol.SetCurSel(m_struIPParaCfgV40.struStreamMode[iIPChanIndex].uGetStream.struIPChan.byTransProtocol);
				m_comboIPStreamType.SetCurSel(m_struIPParaCfgV40.struStreamMode[iIPChanIndex].uGetStream.struIPChan.byTransMode);
			}
		}
		break;
	default: 
...
void CDlgRemoteIPAccess::OnBtnStreamUrlSet() 
{
	...
	iIPChanIndex = m_iCurChanNO - m_struIPParaCfgV40.dwStartDChan - iGroupNO * MAX_CHANNUM_V30;\
	m_iDeviceIdUrl = iIPChanIndex + iGroupNO * MAX_CHANNUM_V30;
...
void CDlgRemoteIPAccess::OnButtonUpdateProList() 
{
	...
	if (NET_DVR_GetIPCProtoList(g_struDeviceInfo[m_dwDevIndex].lLoginID, &m_struProtoList))
	{
		...
		if(m_struIPParaCfgV40.struIPDevInfo[m_iCurChanNO - m_struIPParaCfgV40.dwStartDChan - m_comboChannelGroupNO.GetCurSel() * MAX_CHANNUM_V30].byProType == m_struProtoList.struProto[i].dwType )
		{
...
BOOL CDlgRemoteIPAccess::FindDataByIp(char *lpDevIpAddress,  LPDWORD pdwIPDevIndex,DWORD dwProtoType)
{
	...
	if (m_struProtoList.dwProtoNum < IPC_PROTOCOL_NUM)
	{
		for (int i = 0; i<m_struProtoList.dwProtoNum; i++)
		{
			if (dwProtoType == m_struProtoList.struProto[i].dwType && !strcmp("GB28181",(char*)m_struProtoList.struProto[i].byDescribe))
			{
				bGB28181 = TRUE;
				break;
				//m_csDevIP = m_str
...
void CDlgRemoteIPAccess::OnSelchangeComboProtype() 
{
	...
	if (m_struProtoList.dwProtoNum < IPC_PROTOCOL_NUM)
	{
		for (int i = 0; i<m_struProtoList.dwProtoNum; i++)
		{
			if (dwProtocalTypeValue == m_struProtoList.struProto[i].dwType && !strcmp("GB28181",(char*)m_struProtoList.struProto[i].byDescribe))
			{
