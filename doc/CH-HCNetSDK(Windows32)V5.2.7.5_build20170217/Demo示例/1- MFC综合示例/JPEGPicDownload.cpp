...
UINT SearchPicThread(LPVOID pParam)
{
	...
	if (0)
	{
		NET_DVR_FIND_PICTURE_V40 strFileInfoV40;
		while (1)
		{	
			...
			bRet = NET_DVR_FindNextPicture_V40(RPic->m_lFileHandle, &strFileInfoV40);
...
//#endif //NET_DVR_GET_TRAVERSE_PLANE_DETECTION
    ...
    m_PicTypeCtrl.AddString(szLan);
    m_PicTypeCtrl.SetItemData(36, 0x2a);

    g_StringLanType(szLan, "折线攀高", "Adv ReachHeight");
    m_PicTypeCtrl.AddString(szLan);
    m_PicTypeCtrl.SetItemData(36, 0x2b);

    g_StringLanType(szLan, "如厕超时", "Toilet Tarry");
    m_PicTypeCtrl.AddString(szLan);
    m_PicTypeCtrl.SetItemData(36, 0x2c);
...
void CJPEGPicDownload::OnBnClickedButtonDownloadPictureByTime()
{
    ...
    struDownloadPictureByTimeCond.struStreamInfo.dwChannel = m_iChannel;
