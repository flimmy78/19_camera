...
void CMatPassive::OnSure() 
{
	...
	if (m_bDesk)
	{
        g_struDeviceInfo[pClient->m_iDeviceIndex].struPassiveDecode[0].lPassiveHandle = NET_DVR_MatrixStartPassiveDecode(m_lServerID, iChann/*m_DecChanCombo.GetCurSel() + m_lStartChan*/, &m_PassiveMode);
	}
	else
#endif
	{
        g_struDeviceInfo[pClient->m_iDeviceIndex].struPassiveDecode[sel].lPassiveHandle = NET_DVR_MatrixStartPassiveDecode(m_lServerID, iChann/*m_DecChanCombo.GetCurSel() + m_lStartChan*/, &m_PassiveMode);
	}
    if (g_struDeviceInfo[pClient->m_iDeviceIndex].struPassiveDecode[sel].lPassiveHandle >= 0)
    {
