...
//     {
        char *p ; 
        if (struCardInfo.byCardType == 0 || struCardInfo.byCardType > 9)
        {
            p = pCardType[0];
        }
        else 
...
//     {
        ...
        char szNameTemp[NAME_LEN + 1] = { 0 };
        memcpy(szNameTemp, struCardInfo.byName, sizeof(struCardInfo.byName));
        strItem.Format("%s", szNameTemp);
...
void CDlgGatewayCardCfg::OnBtnGetSel() 
{
    ...
    m_lSetCardCfgHandle = NET_DVR_StartRemoteConfig(m_lServerID, NET_DVR_GET_CARD_CFG_V50, &struCond, sizeof(struCond), g_fGetGatewayCardCallback, this);
    if (m_lSetCardCfgHandle == -1)
    {
        g_pMainDlg->AddLog(m_iDevIndex, OPERATION_FAIL_T, "NET_DVR_GET_CARD_CFG_V50");
...
LPDWORD CDlgGatewayCardCfg::GetCardCfgPoint()
{
    if ( !m_lpRecordCardCfg)
    {
...
void CDlgGatewayCardCfg::CreateSameCfgCard(DWORD dwNum, const NET_DVR_CARD_CFG_V50 &struCardCfg)
{
    ...
    for (i=0; i<dwNum; i++)
    {
        ...
        lpArr[dwCardNumNow+i] = (DWORD)pCardInfo;
    }
}
