...
void CDlgMultiCardCfg::OnBtnGetMultiCard() 
{
	...
	memset(&m_struMultiCardCfg, 0, sizeof(m_struMultiCardCfg));
    if (NET_DVR_GetDVRConfig(m_lUserID, NET_DVR_GET_MULTI_CARD_CFG, m_dwDoorNo, &m_struMultiCardCfg, sizeof(m_struMultiCardCfg), &dwReturn))
    {
        g_pMainDlg->AddLog(m_iDevIndex, OPERATION_SUCC_T, "NET_DVR_GET_MULTI_CARD_CFG");
    }
    else
    {
        g_pMainDlg->AddLog(m_iDevIndex, OPERATION_FAIL_T, "NET_DVR_GET_MULTI_CARD_CFG");
        return;
    }
...
void CDlgMultiCardCfg::OnBtnSetMultiCard() 
{
	// TODO: Add your control notification handler code here
    UpdateData(TRUE); 
    if (NET_DVR_SetDVRConfig(m_lUserID, NET_DVR_SET_MULTI_CARD_CFG, m_dwDoorNo, &m_struMultiCardCfg, sizeof(m_struMultiCardCfg)))
    {
        g_pMainDlg->AddLog(m_iDevIndex, OPERATION_SUCC_T, "NET_DVR_SET_MULTI_CARD_CFG");
    }
    else
    {
        g_pMainDlg->AddLog(m_iDevIndex, OPERATION_FAIL_T, "NET_DVR_SET_MULTI_CARD_CFG");
        return;
    }
