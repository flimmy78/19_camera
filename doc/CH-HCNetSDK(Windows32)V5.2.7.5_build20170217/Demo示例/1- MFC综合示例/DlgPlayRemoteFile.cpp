	...
	while (g_pDlgRemoteFile->m_bQuit)
	{
		...
		else
		{
			...
			if ((lRet == NET_DVR_NOMOREFILE) || (lRet == NET_DVR_FILE_NOFIND))
			{
				...
				g_pMainDlg->AddLog(g_pDlgRemoteFile->m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_FindNextFile_V40 file num[%d]", g_pDlgRemoteFile->m_iFileNum);
...
// 				}
			...
			else
			{
				...
				g_pDlgRemoteFile->m_bSearching = FALSE;
				(g_pDlgRemoteFile->GetDlgItem(IDC_STATIC_REMOTE_SEARCH_STATE))->ShowWindow(SW_HIDE);
                g_pMainDlg->AddLog(g_pDlgRemoteFile->m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_FindNextFile_V40");
...
// 		}
		...
		if (3 == m_comboStreamtype.GetCurSel())
		{
			m_struFileCond.byStreamType = 0xff;
		}
