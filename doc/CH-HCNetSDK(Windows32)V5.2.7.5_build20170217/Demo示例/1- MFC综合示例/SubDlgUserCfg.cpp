...
CSubDlgUserCfg::~CSubDlgUserCfg()
{
    ...
    if (m_pStruUserCfg_V40 != NULL)
    {
        delete m_pStruUserCfg_V40;
        m_pStruUserCfg_V40 = NULL;
    }
}
...
BOOL CSubDlgUserCfg::OnInitDialog()
{
	...
	m_pStruUserCfg_V40 = new NET_DVR_USER_V40;
	if (m_pStruUserCfg_V40 == NULL)
	{
		return FALSE;
	}

	memset(m_pStruUserCfg_V40, 0, sizeof(NET_DVR_USER_V40));
	CRect rc(0, 0, 0, 0);
...
//	else
    ...
    if (!NET_DVR_GetDVRConfig(m_lLoginID, NET_DVR_GET_USERCFG_V50, 0, &m_StruUserCfg_V50, sizeof(m_StruUserCfg_V50), &dwReturned))
	{
		g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_GET_USERCFG_V50");
		char szLan[128] = {0};
	...
	{
		g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_GET_USERCFG_V50");
	}
...
void CSubDlgUserCfg::CurCfgUpdate()
{
	...
	EnableWindow(TRUE);
    memcpy(m_pStruUserCfg_V40->struUser, m_StruUserCfg_V50.struUser, sizeof(m_StruUserCfg_V50.struUser));
	CString csUserName;
	char cTemp[100] = {0};
	int i = 0;
	...
	else
	{
		for (i=0; i<MAX_USERNUM_V30; i++)
		{
			...
			memcpy(cTemp, m_pStruUserCfg_V40->struUser[i].sUserName, MAX_NAMELEN);
...
BOOL CSubDlgUserCfg::CfgSetup()
{
	if (m_bSetUserParams)
	{
        if (!NET_DVR_SetDVRConfig(m_lLoginID, NET_DVR_SET_USERCFG_V50, 0, &m_StruUserCfg_V50, sizeof(m_StruUserCfg_V50)))
		{
			g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_SET_USERCFG_V50"); 
			char szLan[128] = {0};
        ...
        {
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_SET_USERCFG_V50"); 
        }
    }
	return TRUE;
}
...
{
    m_pStruUserCfg_V40->dwSize = sizeof(*m_pStruUserCfg_V40);
	SaveLastLocalChan();
	SaveLastRemoteChan();

	UpdateData(TRUE);
	strncpy((char *)m_pStruUserCfg_V40->struUser[m_iUserIndex].sUserName, m_csUserName, MAX_NAMELEN);
	...
	strncpy((char *)m_pStruUserCfg_V40->struUser[m_iUserIndex].sPassword, m_csUserPwd,PASSWD_LEN);
	if (m_csUserIP.Find(':') != -1)
	{
		strncpy((char *)m_pStruUserCfg_V40->struUser[m_iUserIndex].struUserIP.byIPv6, m_csUserIP, 128);
	}
	else
	{
		strncpy((char *)m_pStruUserCfg_V40->struUser[m_iUserIndex].struUserIP.sIpV4, m_csUserIP, 16);
	}
	int nVal;
	sscanf(m_csMacAddr1, "%02x", &nVal);
	m_pStruUserCfg_V40->struUser[m_iUserIndex].byMACAddr[0] = (BYTE)nVal;
	sscanf(m_csMacAddr2, "%02x", &nVal);
	m_pStruUserCfg_V40->struUser[m_iUserIndex].byMACAddr[1] = (BYTE)nVal;
	sscanf(m_csMacAddr3, "%02x", &nVal);
	m_pStruUserCfg_V40->struUser[m_iUserIndex].byMACAddr[2] = (BYTE)nVal;
	sscanf(m_csMacAddr4, "%02x", &nVal);
	m_pStruUserCfg_V40->struUser[m_iUserIndex].byMACAddr[3] = (BYTE)nVal;
	sscanf(m_csMacAddr5, "%02x", &nVal);
	m_pStruUserCfg_V40->struUser[m_iUserIndex].byMACAddr[4] = (BYTE)nVal;
	sscanf(m_csMacAddr6, "%02x", &nVal);
	m_pStruUserCfg_V40->struUser[m_iUserIndex].byMACAddr[5] = (BYTE)nVal;

	m_pStruUserCfg_V40->struUser[m_iUserIndex].byPriority = (BYTE)m_comboPriority.GetCurSel();
	m_pStruUserCfg_V40->struUser[m_iUserIndex].byLocalRight[0] = (BYTE)m_bChkLocalPTZCtrl;
	m_pStruUserCfg_V40->struUser[m_iUserIndex].byLocalRight[1] = (BYTE)m_bChkLocalRecManul;
	m_pStruUserCfg_V40->struUser[m_iUserIndex].byLocalRight[2] = (BYTE)m_bChkLocalPlayBack;
	m_pStruUserCfg_V40->struUser[m_iUserIndex].byLocalRight[3] = (BYTE)m_bChkLocalSetParam;
	m_pStruUserCfg_V40->struUser[m_iUserIndex].byLocalRight[4] = (BYTE)m_bChkLocalShowStat;
	m_pStruUserCfg_V40->struUser[m_iUserIndex].byLocalRight[5] = (BYTE)m_bChkLocalHighOperation;
	m_pStruUserCfg_V40->struUser[m_iUserIndex].byLocalRight[7] = (BYTE)m_bChkLocalCameraManage;
	m_pStruUserCfg_V40->struUser[m_iUserIndex].byLocalRight[8] = (BYTE)m_bChkLocalBackup;
	m_pStruUserCfg_V40->struUser[m_iUserIndex].byLocalRight[9] = (BYTE)m_bChkLocalReboot;

	m_pStruUserCfg_V40->struUser[m_iUserIndex].byRemoteRight[0] = (BYTE)m_bChkRemotePTZCtrl;
	m_pStruUserCfg_V40->struUser[m_iUserIndex].byRemoteRight[1] = (BYTE)m_bChkRemoteRecManul;
	m_pStruUserCfg_V40->struUser[m_iUserIndex].byRemoteRight[2] = (BYTE)m_bChkRemotePlayBack;
	m_pStruUserCfg_V40->struUser[m_iUserIndex].byRemoteRight[3] = (BYTE)m_bChkRemoteSetParam;
	m_pStruUserCfg_V40->struUser[m_iUserIndex].byRemoteRight[4] = (BYTE)m_bChkRemoteShowStat;
	m_pStruUserCfg_V40->struUser[m_iUserIndex].byRemoteRight[5] = (BYTE)m_bChkRemoteHighOperation;
	m_pStruUserCfg_V40->struUser[m_iUserIndex].byRemoteRight[6] = (BYTE)m_bChkRemoteVoiceTalk;
	m_pStruUserCfg_V40->struUser[m_iUserIndex].byRemoteRight[7] = (BYTE)m_bChkRemotePreview;
	m_pStruUserCfg_V40->struUser[m_iUserIndex].byRemoteRight[8] = (BYTE)m_bChkRemoteAlarm;
	m_pStruUserCfg_V40->struUser[m_iUserIndex].byRemoteRight[9] = (BYTE)m_bChkRemoteLocalOut;
	m_pStruUserCfg_V40->struUser[m_iUserIndex].byRemoteRight[10] = (BYTE)m_bChkRemoteSerialCtrl;
	m_pStruUserCfg_V40->struUser[m_iUserIndex].byRemoteRight[12] = (BYTE)m_bChkNetCameraManage;
	m_pStruUserCfg_V40->struUser[m_iUserIndex].byRemoteRight[13] = (BYTE)m_bChkNetReboot;
    m_pStruUserCfg_V40->struUser[m_iUserIndex].byAlarmOnRight = m_bAlarmOn;
    m_pStruUserCfg_V40->struUser[m_iUserIndex].byAlarmOffRight = m_bAlarmOff;
    m_pStruUserCfg_V40->struUser[m_iUserIndex].byBypassRight = m_bBypass;
    m_pStruUserCfg_V40->struUser[m_iUserIndex].byPublishRight[0] = (BYTE)m_bMaterial;
    m_pStruUserCfg_V40->struUser[m_iUserIndex].byPublishRight[1] = (BYTE)m_bProgram;
    m_pStruUserCfg_V40->struUser[m_iUserIndex].byPublishRight[2] = (BYTE)m_bSchedule;
    m_pStruUserCfg_V40->struUser[m_iUserIndex].byPublishRight[3] = (BYTE)m_bMaterialUpload;
    m_pStruUserCfg_V40->struUser[m_iUserIndex].byPublishRight[4] = (BYTE)m_bProgramAdd;
    m_pStruUserCfg_V40->struUser[m_iUserIndex].byPublishRight[5] = (BYTE)m_bScheduleAdd;
    m_pStruUserCfg_V40->struUser[m_iUserIndex].byPublishRight[6] = (BYTE)m_bPlay;
    m_pStruUserCfg_V40->struUser[m_iUserIndex].byPublishRight[7] = (BYTE)m_bTerminal;
    m_pStruUserCfg_V40->struUser[m_iUserIndex].byPublishRight[8] = (BYTE)m_bShop;

	//OnBnClickedBtnLocalChanOk();
//	OnBnClickedBtnRemoteChanOk();
    strncpy((char *)m_StruUserCfg_V50.sloginPassword, m_sloginPassword, PASSWD_LEN);
    m_StruUserCfg_V50.dwSize = sizeof(m_StruUserCfg_V50);
    m_StruUserCfg_V50.dwMaxUserNum = m_pStruUserCfg_V40->dwMaxUserNum;
    memcpy((char *)m_StruUserCfg_V50.struUser, (char *)m_pStruUserCfg_V40->struUser, sizeof(m_pStruUserCfg_V40->struUser));
...
void CSubDlgUserCfg::OnCbnSelchangeComboUserSel()
{
	...
	ZeroMemory(cTemp, 100);
	memcpy(cTemp, m_pStruUserCfg_V40->struUser[m_iUserIndex].sUserName, MAX_NAMELEN);
	m_csUserName.Format("%s", cTemp);
	ZeroMemory(cTemp, 100);
	memcpy(cTemp, m_pStruUserCfg_V40->struUser[m_iUserIndex].sPassword, PASSWD_LEN);
	m_csUserPwd.Format("%s", cTemp);
	ZeroMemory(cTemp, 100);
	memcpy(cTemp, m_pStruUserCfg_V40->struUser[m_iUserIndex].sPassword, PASSWD_LEN);
	m_csVerifyPwd.Format("%s", cTemp);

	if (strlen((char *)m_pStruUserCfg_V40->struUser[m_iUserIndex].struUserIP.byIPv6) != 0 && strlen((char *)m_pStruUserCfg_V40->struUser[m_iUserIndex].struUserIP.byIPv6) != 2)
	{
		m_csUserIP.Format("%s", m_pStruUserCfg_V40->struUser[m_iUserIndex].struUserIP.byIPv6);
	}
	else
	{
		m_csUserIP.Format("%s", m_pStruUserCfg_V40->struUser[m_iUserIndex].struUserIP.sIpV4);
	}
	m_csMacAddr1.Format("%02x",m_pStruUserCfg_V40->struUser[m_iUserIndex].byMACAddr[0]);
	m_csMacAddr2.Format("%02x",m_pStruUserCfg_V40->struUser[m_iUserIndex].byMACAddr[1]);
	m_csMacAddr3.Format("%02x",m_pStruUserCfg_V40->struUser[m_iUserIndex].byMACAddr[2]);
	m_csMacAddr4.Format("%02x",m_pStruUserCfg_V40->struUser[m_iUserIndex].byMACAddr[3]);
	m_csMacAddr5.Format("%02x",m_pStruUserCfg_V40->struUser[m_iUserIndex].byMACAddr[4]);
	m_csMacAddr6.Format("%02x",m_pStruUserCfg_V40->struUser[m_iUserIndex].byMACAddr[5]);	
	m_bChkLocalPTZCtrl = m_pStruUserCfg_V40->struUser[m_iUserIndex].byLocalRight[0];
	m_bChkLocalRecManul = m_pStruUserCfg_V40->struUser[m_iUserIndex].byLocalRight[1];
	m_bChkLocalPlayBack = m_pStruUserCfg_V40->struUser[m_iUserIndex].byLocalRight[2];
	m_bChkLocalSetParam = m_pStruUserCfg_V40->struUser[m_iUserIndex].byLocalRight[3];
	m_bChkLocalShowStat = m_pStruUserCfg_V40->struUser[m_iUserIndex].byLocalRight[4];
	m_bChkLocalHighOperation = m_pStruUserCfg_V40->struUser[m_iUserIndex].byLocalRight[5];
	m_bChkLocalCameraManage = m_pStruUserCfg_V40->struUser[m_iUserIndex].byLocalRight[7];
	m_bChkLocalBackup = m_pStruUserCfg_V40->struUser[m_iUserIndex].byLocalRight[8];
	m_bChkLocalReboot = m_pStruUserCfg_V40->struUser[m_iUserIndex].byLocalRight[9];

	m_bChkRemotePTZCtrl = m_pStruUserCfg_V40->struUser[m_iUserIndex].byRemoteRight[0];
	m_bChkRemoteRecManul = m_pStruUserCfg_V40->struUser[m_iUserIndex].byRemoteRight[1];
	m_bChkRemotePlayBack = m_pStruUserCfg_V40->struUser[m_iUserIndex].byRemoteRight[2];
	m_bChkRemoteSetParam = m_pStruUserCfg_V40->struUser[m_iUserIndex].byRemoteRight[3];
	m_bChkRemoteShowStat = m_pStruUserCfg_V40->struUser[m_iUserIndex].byRemoteRight[4];
	m_bChkRemoteHighOperation = m_pStruUserCfg_V40->struUser[m_iUserIndex].byRemoteRight[5];
	m_bChkRemoteVoiceTalk = m_pStruUserCfg_V40->struUser[m_iUserIndex].byRemoteRight[6];
	m_bChkRemotePreview = m_pStruUserCfg_V40->struUser[m_iUserIndex].byRemoteRight[7];
	m_bChkRemoteAlarm = m_pStruUserCfg_V40->struUser[m_iUserIndex].byRemoteRight[8];
	m_bChkRemoteLocalOut = m_pStruUserCfg_V40->struUser[m_iUserIndex].byRemoteRight[9];
	m_bChkRemoteSerialCtrl = m_pStruUserCfg_V40->struUser[m_iUserIndex].byRemoteRight[10];
	m_bChkNetCameraManage = m_pStruUserCfg_V40->struUser[m_iUserIndex].byRemoteRight[12];
	m_bChkNetReboot = m_pStruUserCfg_V40->struUser[m_iUserIndex].byRemoteRight[13];
    m_bAlarmOn = m_pStruUserCfg_V40->struUser[m_iUserIndex].byAlarmOnRight;
    m_bAlarmOff = m_pStruUserCfg_V40->struUser[m_iUserIndex].byAlarmOffRight;
    m_bBypass = m_pStruUserCfg_V40->struUser[m_iUserIndex].byBypassRight;

    m_bMaterial = m_pStruUserCfg_V40->struUser[m_iUserIndex].byPublishRight[0];
    m_bProgram = m_pStruUserCfg_V40->struUser[m_iUserIndex].byPublishRight[1];
    m_bSchedule = m_pStruUserCfg_V40->struUser[m_iUserIndex].byPublishRight[2];
    m_bMaterialUpload = m_pStruUserCfg_V40->struUser[m_iUserIndex].byPublishRight[3];
    m_bProgramAdd = m_pStruUserCfg_V40->struUser[m_iUserIndex].byPublishRight[4];
    m_bScheduleAdd = m_pStruUserCfg_V40->struUser[m_iUserIndex].byPublishRight[5];
    m_bPlay = m_pStruUserCfg_V40->struUser[m_iUserIndex].byPublishRight[6];
    m_bTerminal = m_pStruUserCfg_V40->struUser[m_iUserIndex].byPublishRight[7];
    m_bShop = m_pStruUserCfg_V40->struUser[m_iUserIndex].byPublishRight[8];
    ...
    {
        if( (m_pStruUserCfg_V40->struUser[m_iUserIndex].dwLocalPlaybackRight[i]) != 0xffffffff && (m_pStruUserCfg_V40->struUser[m_iUserIndex].dwLocalPlaybackRight[i] < MAX_CHANNUM_V40))
        {
            m_pbLocalChan[m_pStruUserCfg_V40->struUser[m_iUserIndex].dwLocalPlaybackRight[i] - 1] = (BYTE)1;
        }
    }
	...
	for (i=0; i<MAX_CHANNUM_V40; i++)
	{
		if ((m_pStruUserCfg_V40->struUser[m_iUserIndex].dwNetPlaybackRight[i] != 0xffffffff) && (m_pStruUserCfg_V40->struUser[m_iUserIndex].dwNetPlaybackRight[i] < MAX_CHANNUM_V40))
		{
			m_pbRemoteChan[m_pStruUserCfg_V40->struUser[m_iUserIndex].dwNetPlaybackRight[i] - 1] = (BYTE)1;
		}
	}
	char szLan[128] = {0};
	...
	m_comboPriority.SetCurSel(m_pStruUserCfg_V40->struUser[m_iUserIndex].byPriority);
	CreateLocalTree();
	CreateRemoteTree();
	UpdateData(FALSE);
}
...
void CSubDlgUserCfg::OnCbnSelchangeComboLocalRightType()
{
	...
	case 0:
		...
		for (i=0;i<MAX_CHANNUM_V40;i++)
		{
			if (m_pStruUserCfg_V40->struUser[m_iUserIndex].dwLocalPlaybackRight[i] != 0xffffffff)
			{
				byRight[m_pStruUserCfg_V40->struUser[m_iUserIndex].dwLocalPlaybackRight[i]-1] = (BYTE)1;
			}
		}
		break;
	case 1:
		...
		for (i=0;i<MAX_CHANNUM_V40;i++)
		{
			if (m_pStruUserCfg_V40->struUser[m_iUserIndex].dwLocalRecordRight[i] != 0xffffffff)
			{
				byRight[m_pStruUserCfg_V40->struUser[m_iUserIndex].dwLocalRecordRight[i]-1] = (BYTE)1;
			}
		}
		break;
	case 2:
		...
		for (i=0;i<MAX_CHANNUM_V40;i++)
		{
			if (m_pStruUserCfg_V40->struUser[m_iUserIndex].dwLocalPTZRight[i] != 0xffffffff)
			{
				byRight[m_pStruUserCfg_V40->struUser[m_iUserIndex].dwLocalPTZRight[i]-1] = (BYTE)1;
			}
		}
		break;
	case 3:
		...
		for (i=0;i<MAX_CHANNUM_V40;i++)
		{
			if (m_pStruUserCfg_V40->struUser[m_iUserIndex].dwLocalBackupRight[i] != 0xffffffff)
			{
				byRight[m_pStruUserCfg_V40->struUser[m_iUserIndex].dwLocalBackupRight[i]-1] = (BYTE)1;
			}
		}
		break;			
	default:
...
void CSubDlgUserCfg::OnCbnSelchangeComboRemoteRightType()
{
	...
	case 0:
		...
		for (i=0;i<MAX_CHANNUM_V40;i++)
		{
			if (m_pStruUserCfg_V40->struUser[m_iUserIndex].dwNetPlaybackRight[i] != 0xffffffff)
			{
				byRight[m_pStruUserCfg_V40->struUser[m_iUserIndex].dwNetPlaybackRight[i]-1] = (BYTE)1;
			}
		}
		break;
	case 1:
		...
		for (i=0;i<MAX_CHANNUM_V40;i++)
		{
			if (m_pStruUserCfg_V40->struUser[m_iUserIndex].dwNetPreviewRight[i] != 0xffffffff)
			{
				byRight[m_pStruUserCfg_V40->struUser[m_iUserIndex].dwNetPreviewRight[i]-1] = (BYTE)1;
			}
		}
		break;
	case 2:
		...
		for (i=0;i<MAX_CHANNUM_V40;i++)
		{
			if (m_pStruUserCfg_V40->struUser[m_iUserIndex].dwNetRecordRight[i] != 0xffffffff)
			{
				byRight[m_pStruUserCfg_V40->struUser[m_iUserIndex].dwNetRecordRight[i]-1] = (BYTE)1;
			}
		}
		break;
	case 3:
		...
		for (i=0;i<MAX_CHANNUM_V40;i++)
		{
			if (m_pStruUserCfg_V40->struUser[m_iUserIndex].dwNetPTZRight[i] != 0xffffffff)
			{
				byRight[m_pStruUserCfg_V40->struUser[m_iUserIndex].dwNetPTZRight[i]-1] = (BYTE)1;
			}
		}
		break;
	default:
...
void CSubDlgUserCfg::OnBnClickedBtnLocalChanOk()
{
	...
	{
	case 0:
        memset(m_pStruUserCfg_V40->struUser[m_iUserIndex].dwLocalPlaybackRight,0xffffffff, sizeof(DWORD)*MAX_CHANNUM_V40);
        for (i=0; i<MAX_CHANNUM_V40; i++)
        {
            
            if (m_pbLocalChan[i] == (BYTE)1)
            {
                m_pStruUserCfg_V40->struUser[m_iUserIndex].dwLocalPlaybackRight[j++] = i+1;
            }
        }
		break;
	case 1:
        memset(m_pStruUserCfg_V40->struUser[m_iUserIndex].dwLocalRecordRight,0xffffffff, sizeof(DWORD)*MAX_CHANNUM_V40);
        for (i=0; i<MAX_CHANNUM_V40; i++)
        {		
            if (m_pbLocalChan[i] == (BYTE)1)
            {
                m_pStruUserCfg_V40->struUser[m_iUserIndex].dwLocalRecordRight[j++] = i+1;
            }
		}
		break;
	case 2:
        memset(m_pStruUserCfg_V40->struUser[m_iUserIndex].dwLocalPTZRight,0xffffffff, sizeof(DWORD)*MAX_CHANNUM_V40);

		for (i=0; i<MAX_CHANNUM_V40; i++)
		{
            if (m_pbLocalChan[i] == (BYTE)1)
			{
				m_pStruUserCfg_V40->struUser[m_iUserIndex].dwLocalPTZRight[j++] = i+1;
			}
		}
	
		break;
	case 3:
        memset(m_pStruUserCfg_V40->struUser[m_iUserIndex].dwLocalBackupRight,0xffffffff, sizeof(DWORD)*MAX_CHANNUM_V40);

		for (i=0; i<MAX_CHANNUM_V40; i++)
		{
            if (m_pbLocalChan[i] == (BYTE)1)
			{
				m_pStruUserCfg_V40->struUser[m_iUserIndex].dwLocalBackupRight[j++] = i+1;
			}
		}
		break;
	default:
...
void CSubDlgUserCfg::OnBnClickedBtnRemoteChanOk()
{
	...
	{
	case 0:
        memset(m_pStruUserCfg_V40->struUser[m_iUserIndex].dwNetPlaybackRight, 0xffffffff, sizeof(DWORD)*MAX_CHANNUM_V40);
		for (i=0; i<MAX_CHANNUM_V40; i++)
		{
			if (byRight[i] == (BYTE)1)
			{
				m_pStruUserCfg_V40->struUser[m_iUserIndex].dwNetPlaybackRight[j++] = i+1;
			}
		}
		break;
	case 1:
        memset(m_pStruUserCfg_V40->struUser[m_iUserIndex].dwNetPreviewRight, 0xffffffff, sizeof(DWORD)*MAX_CHANNUM_V40);
		for (i=0; i<MAX_CHANNUM_V40; i++)
		{
			if (byRight[i] == (BYTE)1)
			{
				m_pStruUserCfg_V40->struUser[m_iUserIndex].dwNetPreviewRight[j++] = i+1;
			}
		}
		break;
    case 2:
        memset(m_pStruUserCfg_V40->struUser[m_iUserIndex].dwNetRecordRight, 0xffffffff, sizeof(DWORD)*MAX_CHANNUM_V40);
		for (i=0; i<MAX_CHANNUM_V40; i++)
		{
			if (byRight[i] == (BYTE)1)
			{
				m_pStruUserCfg_V40->struUser[m_iUserIndex].dwNetRecordRight[j++] = i+1;
			}
		}
		break;
	case 3:
        memset(m_pStruUserCfg_V40->struUser[m_iUserIndex].dwNetPTZRight, 0xffffffff, sizeof(DWORD)*MAX_CHANNUM_V40);
		for (i=0; i<MAX_CHANNUM_V40; i++)
		{
			if (byRight[i] == (BYTE)1)
			{
				m_pStruUserCfg_V40->struUser[m_iUserIndex].dwNetPTZRight[j++] = i+1;
			}
		}
		break;
	default:
