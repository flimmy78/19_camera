	...
	while(hChild!=NULL)
	{
		...
		if(bChk)
		{
			...
			{   
				...
				g_struDeviceInfo[iDevIndex].lFortifyHandle = NET_DVR_SetupAlarmChan_V41(g_struDeviceInfo[iDevIndex].lLoginID,&struSetupAlarmParam);
				if(g_struDeviceInfo[iDevIndex].lFortifyHandle == -1)
				{	
					m_treeAll.SetItemImage(hChild,0,0);
					g_pMainDlg->AddLog(iDevIndex, OPERATION_FAIL_T, "NET_DVR_SetupAlarmChan_V41");
					g_StringLanType(szLan, "报警设防失败", "Setup alarm chan failed");
					AfxMessageBox(szLan);
				}
				else
				{
					g_pMainDlg->AddLog(iDevIndex, OPERATION_SUCC_T, "NET_DVR_SetupAlarmChan_V41");
				}
			}
