...
BOOL CDlgAcsFaceParamCfg::OnInitDialog()
{
    ...
    m_listFaceParamCfg.InsertColumn(2, szLan, LVCFMT_LEFT, 200, -1);
    g_StringLanType(szLan, "人脸编号", "Face No");
    m_listFaceParamCfg.InsertColumn(3, szLan, LVCFMT_LEFT, 80, -1);
    g_StringLanType(szLan, "人脸数据类型", "Face Data TYPE");
	m_listFaceParamCfg.InsertColumn(4, szLan, LVCFMT_LEFT, 80, -1);
...
LRESULT CDlgAcsFaceParamCfg::OnMsgGetFaceParamCfgFinish(WPARAM wParam, LPARAM lParam)
{
    ...
    m_lGetFaceParamCfgHandle = -1;
    g_pMainDlg->AddLog(m_iDevIndex, OPERATION_SUCC_T, "NET_DVR_GET_FACE_PARAM_CFG Get finish");
    return 0;
}
