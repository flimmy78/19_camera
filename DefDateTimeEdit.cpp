	...
	: QDateTimeEdit(parent)
{
    ...
    this->setCalendarWidget(m_DefCalendar);
...
{
    if(NULL != m_DefCalendar ) {
        delete m_DefCalendar;
        m_DefCalendar = NULL;
    }
}
...
{
    emit setFinished(dateTime);
}

void DefDateTimeEdit::setStartDateTime(const QDateTime &dateTime)
{
    //set start time less than end time
    this->setMaximumDateTime(dateTime);
}

///<summary> Get Section DateTime</summary>
void DefDateTimeEdit::getSectionTextDateTime( QString& year, QString&month, \
                                      QString& day, QString& hour, QString& minute,QString& second)
{
    year = this->sectionText(QDateEdit::YearSection);
    month = this->sectionText(QDateEdit::MonthSection);
    day = this->sectionText(QDateEdit::DaySection);
    hour = this->sectionText(QDateEdit::HourSection);
    minute = this->sectionText(QDateEdit::MinuteSection);
    second = this->sectionText(QDateEdit::SecondSection);
}

///<summary> Setting calendar stytle</summary>
void DefDateTimeEdit::setMyStytle()
{
    ...
    strTemp.append("QComboBox#CalComboBox{border:0px; background-color:rgb(0, 159, 149); \
                   color:white; height:24px; width:43px;}\
    ...
    strTemp.append("QDateTimeEdit::drop-down{width:15px; border:0px solid red;\
